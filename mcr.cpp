...
bool test(int i, int j, char game[3][3]){
	bool available = false;
	if(game[i][j]!='X' && game[i][j]!='O') available = true;
	return available;
}
int main() {
    int i, j;
    ...
    for (int n = 0; n < 9; n++) {
        ...
        while(1){
			cin >> i >> j;
        	if(test(i,j,game))  break;
        	else  cout << "The cell has already taken, please enter another cell to mark:"; 
    	}
		if (turn == false) 
            game[i][j] = 'X';
        else
